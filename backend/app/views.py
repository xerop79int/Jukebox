...
class ManagerUploadSongsListView(APIView):
    ...
    def write_pdf_to_images(self, pdf_file, folder_name):
        pages = extract_pages(pdf_file)
        iw = ImageWriter(self.OUTPUT_DIR + folder_name)
    ...
    def replace_weird_chord_characters(self, text):
        return text.replace('Gc', 'G').replace('Cc', 'C')

    def left_strip(self, body_list):
        index = 0
    ...
    def add_new_lines(self, body_list):
        index = 0
    ...
    def format_body_string(self, body_string):
        ...
        formatted_body_list = self.add_new_lines(self.left_strip(body_list))
    ...
    def get_body(self, text):
        ...
        body_string = self.format_body_string(body_string)
        return body_string


    def read_pdf_file(self, pdf_file):
        output_folder_name = pdf_file[0:pdf_file.find('.pdf')]
        output_directory = self.write_pdf_to_images(pdf_file, output_folder_name)
        output_files = [output_directory + '/' + file for file in os.listdir(output_directory)]
        text = ''
        for file in output_files:
            text += self.convert_image_to_text(file)
        shutil.rmtree(self.OUTPUT_DIR)
        return text


    def write_to_text_file(self, pdf_file):
        text = self.read_pdf_file(pdf_file)
        text = self.extract_page_numbers(text)
        text = self.replace_weird_chord_characters(text)
